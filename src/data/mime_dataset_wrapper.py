...
class MimeDataSet(Dataset):
    ...
    def __init__(self,
                 ...
                 hd_kinect_depth_data: bool = False,
                 rd_kinect_img_data: bool = False,
                 rd_kinect_depth_data: bool = False,
                 rd_sk_left_img_data: bool = False,
                 rd_sk_left_depth_data: bool = False,
                 rd_sk_right_img_data: bool = False,
                 rd_sk_right_depth_data: bool = False,
        ...
        :param start_ind: Index to start sampling from (Between 0 and -2)
        :param stop_ind: Maximal index to sample from (Between 1 and -1)
        ...
        self.base_paths = [join(self.base_path, sub_path) for sub_path in listdir(self.base_path)][start_ind:stop_ind]
        ...
        self.hd_kinect_depth_data = hd_kinect_depth_data
        self.rd_kinect_depth_data = rd_kinect_depth_data
        self.rd_sk_right_depth_data = rd_sk_right_depth_data
        self.rd_sk_left_depth_data = rd_sk_left_depth_data
        ...
        self.trajectory_lengths = self.trajectory_lengths[start_ind:stop_ind]
    ...
    def read_hd_kinect_img_data(self, index: int):
        hd_kinect_img_series = None
        try:
            # NOTE: The following are in (T, H, W, C) format
            hd_kinect_img_series = read_video(self.hd_kinect_rgb_paths[index], pts_unit='sec')[0]
            ...
            faulty_path = self.hd_kinect_rgb_paths[self.map_index_to_slice_stack(index)[0]]
            print(f"\n\n\nCould not read hd kinect data at {faulty_path}:")
            print(e)
            print("\n\n\n")

        return hd_kinect_img_series

    def read_rd_kinect_img_data(self, index: int):
        rd_kinect_img_series = None
        try:
            # NOTE: The following are in (T, H, W, C) format
            rd_kinect_img_series = read_video(self.rd_kinect_rgb_paths[index], pts_unit='sec')[0]
            rd_kinect_img_series = rd_kinect_img_series.permute(0, 3, 1, 2)
            rd_kinect_img_series = rd_kinect_img_series.float() / 255.0
            if self.scale_factor != 1.0:
                rd_kinect_img_series = interpolate(rd_kinect_img_series,
                                                   scale_factor=self.scale_factor,
                                                   recompute_scale_factor=False)
    ...
    def __getitem__(self, index) -> T_co:
        ...
        # TODO: Handle potential exceptions in the read function
        ...
        if self.hd_kinect_img_data:
            hd_kinect_img_series = self.read_hd_kinect_img_data(sample_index)
            assert hd_kinect_img_series is not None
            hd_kinect_img_series = hd_kinect_img_series[stack_start_index:stack_stop_index, ...]

        if self.rd_kinect_img_data:
            rd_kinect_img_series = self.read_rd_kinect_img_data(sample_index)
            assert rd_kinect_img_series is not None
            rd_kinect_img_series = rd_kinect_img_series[stack_start_index:stack_stop_index, ...]

        # TODO: Find a better way than returning a dict of tensors
