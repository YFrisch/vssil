...
def play_series_and_reconstruction_with_keypoints(image_series: torch.Tensor,
    ...
    """ Visualizes the an image-series tensor
        ...
        TODO: Handle shape[-1]==2 for the key-points (always active)

    :param image_series: Tensor of images in (N, T, C, H, W)
    :param keypoint_coords: Tensor of key-point coordinates in (N, T, K, 3)
    ...
    reconstructed_diff_series = (reconstructed_diff.permute(0, 1, 3, 4, 2) + 0.5).clip(0.0, 1.0).detach().cpu().numpy()
    ...
    for n_keypoint in range(keypoint_coords.shape[2]):
        ...
        if intensity > 0.0:
            orig_scatter_obj = ax[0].scatter(0, 0, cmap=viridis)
    ...
    def animate(t: int):
        ...
        for n_keypoint in range(keypoint_coords.shape[2]):
            ...
            x1 = int((-keypoint_coords[0, t, n_keypoint, 1] + 1.0) / 2 * H)
            x2 = int((keypoint_coords[0, t, n_keypoint, 0] + 1.0)/2 * W)
        ...
        return im_frame, rec_frame, rec_diff_frame, orig_scatter_objects, rec_scatter_objects
...
def gen_eval_imgs(sample: torch.Tensor,
    ...
    :param reconstruction: Torch tensor of predicted differences from t0 to tT in (N, T, C, H, W)
    :param key_points: Torch tensor of key-point coordinates in (N, T, C, 3) or (N, T, C, 2)
    :return: Series of images as torch tensor in (1, T, C, H, W)
    """
    ...
    assert sample.shape[0] == 1, 'Use batch size = 1'
    ...
    for t in range(sample.shape[1]):
        ...
        for kp in range(key_points.shape[2]):
            if key_points.shape[-1] == 2 or key_points[0, t, kp, 2] > 0.5:
                key_point_x = int(((-key_points[0, t, kp, 1] + 1.0) / 2.0) * sample.shape[2])
                key_point_y = int(((key_points[0, t, kp, 0] + 1.0) / 2.0) * sample.shape[3])
                ax[0].scatter(key_point_x, key_point_y, marker='o', cmap=viridis)
        ...
        ax[3].imshow((reconstruction[0, t, ...] + 0.5).clip(0.0, 1.0).permute(1, 2, 0).cpu().numpy())
